...
class ArithmeticOperator_Response(metaclass=Metaclass_ArithmeticOperator_Response):
    ...
    def __init__(self, **kwargs):
        assert all('_' + key in self.__slots__ for key in kwargs.keys()), \
            'Invalid arguments passed to constructor: %s' % \
            ', '.join(sorted(k for k in kwargs.keys() if '_' + k not in self.__slots__))
        self.artimetic_result = kwargs.get('artimetic_result', float())
    ...
    def __eq__(self, other):
        if not isinstance(other, self.__class__):
            return False
        if self.artimetic_result != other.artimetic_result:
            return False
        return True
    ...
    @builtins.property
    def artimetic_result(self):
        """Message field 'artimetic_result'."""
        return self._artimetic_result

    @artimetic_result.setter
    def artimetic_result(self, value):
        if __debug__:
            assert \
                isinstance(value, float), \
                "The 'artimetic_result' field must be of type 'float'"
            assert not (value < -3.402823466e+38 or value > 3.402823466e+38) or math.isinf(value), \
                "The 'artimetic_result' field must be a float in [-3.402823466e+38, 3.402823466e+38]"
        self._artimetic_result = value
